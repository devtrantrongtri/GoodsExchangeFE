...
import { useChangeProductStatusMutation } from "../../services/product/product.service";

const Report = () => {
  const { data, error, isLoading, refetch } = useGetAllReportQuery();
  ...
  const [selectedProducts, setSelectedReports] = useState<number[]>([]);
  ...
  const handleAcceptReport = async (productId: number, reportId: number) => {
    try {
      console.log(productId);
      const productResponse = await changeProductStatus({
        id: productId,
        status: "BANNED",
      });
      console.log("Product Status Response:", productResponse);

      const reportResponse = await changeReportStatus({
        id: reportId,
        status: "RESOLVED",
      });
  ...
  const toggleSelectAll = () => {
    ...
    if (!selectAll) {
      const allReportIds = reports.map((report) => report.report_id);
  ...
  const toggleSelectProduct = (reportId: number) => {
  ...
  // Check if report_title exists before calling toLowerCase
              ...
              currentReports.map((report) => (
                ...
                >
                  <td className="w-4 p-4">
                    <div className="flex items-center">
                      <input
                        id={`checkbox-product-${report.report_id}`}
                        type="checkbox"
                        checked={selectedProducts.includes(report.report_id)}
                        onChange={() => toggleSelectProduct(report.report_id)}
                        className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 light:focus:ring-blue-600 light:ring-offset-gray-800 light:focus:ring-offset-gray-800 focus:ring-2 light:bg-gray-700 light:border-gray-600"
                      />
                      <label
                        htmlFor={`checkbox-product-${report.report_id}`}
                        className="sr-only"
                      >
                        checkbox
                      </label>
                  ...
                  <td className="px-6 py-4">
                    <img src={report.reportImg} alt="" className="w-16 h-16" />
                  </td>
                  <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap light:text-white">
                    {report.reportedBy.userId}
                  </td>
                  <td className="px-6 py-4">{report.product.title}</td>
                  <td className="px-6 py-4">{report.reportTitle}</td>
                  <td className="px-6 py-4">{report.reportReason}</td>
                  <td className="px-6 py-4">
                    {formatDateFromTimestamp(report.report_date)}
                  </td>
                  <td className="px-6 py-4">
                    <span
                      className={`inline-block px-2 py-1 my-1 text-xs font-semibold text-white rounded ${
                        report.status.trim().toLowerCase() === "pending"
                          ? "bg-green-500"
                          : "bg-red-500"
                      }`}
                    >
